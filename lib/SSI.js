...
(function() {
	...
	var ssi = function(inputDirectory, outputDirectory, matcher, inputEncoding, outputEncoding) {
		this.inputDirectory = inputDirectory;
		this.documentRoot = inputDirectory;
		this.outputDirectory = outputDirectory;
		this.matcher = matcher;
    this.inputEncoding = inputEncoding || 'utf8';
    this.outputEncoding = outputEncoding || 'utf8';
	...
	ssi.prototype = {
		...
		parse: function(filename, contents, variables) {
			...
			contents = contents.replace(new RegExp(DIRECTIVE_MATCHER), function(directive, directiveName) {
				var data = instance.directiveHandler.handleDirective(directive, directiveName, filename, variables, instance.inputEncoding, instance.outputEncoding);
